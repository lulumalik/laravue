<template>
  <div>
    <aside class="relative bg-sidebar h-screen w-64 hidden sm:block shadow-xl pr-0">
      <!-- <div v-for="(img, index) in datazakat" :key="index">
        <img :src="'/data_file/' + img.file" />
      </div> -->
      <div class="p-6">
        <a
          href="index.html"
          class="text-white text-3xl font-semibold uppercase hover:text-gray-300"
        >Admin</a>
        <button
          class="w-full bg-white cta-btn font-semibold py-2 mt-5 rounded-br-lg rounded-bl-lg rounded-tr-lg shadow-lg hover:shadow-xl hover:bg-gray-300 flex items-center justify-center"
        >
          <i class="fas fa-plus mr-3"></i> New Report
        </button>
      </div>
      <nav class="text-white text-base font-semibold pt-3 px-6">
        <a
          href="index.html"
          class="flex items-center active-nav-link rounded-full text-white py-4 pl-6 nav-item"
        >
          <i class="fas fa-tachometer-alt mr-3"></i>
          Dashboard
        </a>
        <a
          href="blank.html"
          class="flex items-center text-white opacity-75 mt-2 hover:opacity-100 rounded-full py-4 pl-6 nav-item"
        >
          <i class="fas fa-sticky-note mr-3"></i>
          Blank Page
        </a>
        <a
          href="tables.html"
          class="flex items-center text-white opacity-75 mt-2 hover:opacity-100 rounded-full py-4 pl-6 nav-item"
        >
          <i class="fas fa-table mr-3"></i>
          Tables
        </a>
        <a
          href="forms.html"
          class="flex items-center text-white opacity-75 mt-2 hover:opacity-100 rounded-full py-4 pl-6 nav-item"
        >
          <i class="fas fa-align-left mr-3"></i>
          Forms
        </a>
        <a
          href="tabs.html"
          class="flex items-center text-white opacity-75 mt-2 hover:opacity-100 rounded-full py-4 pl-6 nav-item"
        >
          <i class="fas fa-tablet-alt mr-3"></i>
          Tabbed Content
        </a>
        <a
          href="calendar.html"
          class="flex items-center text-white opacity-75 mt-2 hover:opacity-100 rounded-full py-4 pl-6 nav-item"
        >
          <i class="fas fa-calendar mr-3"></i>
          Calendar
        </a>
      </nav>
      <!-- <div class="flex items-center justify-center">
        <a
          href="calendar.html"
          class="flex items-center justify-start active-nav-link text-white rounded-full py-4 pl-4 nav-item pr-8"
        >
          <i class="fas fa-arrow-circle-up mr-3"></i>
          Upgrade to Pro!
        </a>
      </div> -->
    </aside>
  </div>
</template>

<script>
// export default {
//   data() {
//     return {
//       datazakat: []
//     }
//   },
//   mounted() {
//     this.$axios.get(`${this.$baseUrl}zakat`).then((res) => {
//       this.datazakat = res.data
//     })
//   }
// }
</script>
